...
const jadibot = async (alpha, m, from) => {
const { sendImage, sendMessage } = alpha;
...
const alpha = await makeWaSocket({
auth: state,
browser: [`JER OFC`, "Chrome", "1.0.0"],
logger: log({ level: "silent" }),
version,
})

alpha.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

alpha.ws.on('CB:call', async (json) => {
...
let qutsnya = await alpha.sendContact(callerId, owner)
await alpha.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await alpha.updateBlockStatus(callerId, "block")
}
})
...
if (!alpha.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(alpha, kay, store)
require('./jerofc')(alpha, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
alpha.ev.on("connection.update", async up => {
...
if (connection == "open") {
alpha.id = alpha.decodeJid(alpha.user.id)
alpha.time = Date.now()
global.conns.push(alpha)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${alpha.decodeJid(alpha.user.id)}*_`)
user = `${alpha.decodeJid(alpha.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); alpha.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); alpha.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); alpha.logout(); }
...
alpha.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: alpha.waUploadToServer })
...
return alpha.relayMessage(jid, tod.message, {messageId: tod.key.id})
} 

alpha.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
alpha.relayMessage(jid, template.message, { messageId: template.key.id })
}

alpha.sendButImg = async (jid, path, teks, fke, but) => {
...
alpha.sendMessage(jid, fjejfjjjer, { quoted: m })
}
